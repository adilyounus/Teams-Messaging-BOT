...
namespace Teams_Messaging_BOT
{
    class Program
    {
        ...
        static string ErrorNorifySMSNo;
        ...
        private static void UpdateGlobalVars()
        {
            ...
            ErrorNorifySMSNo = settings["ErrorNorifySMSNo"];
        ...
        private static void NotifyErrorToTeam(string FilePath, string exMessage, string MessageContent)
        {
            ...
            catch (Exception iex)
            {
                ...
                if (!string.IsNullOrEmpty(ErrorNorifySMSNo))
                    SendSMS(ErrorNorifySMSNo, $"Unable to send message to teams, FileName: {Path.GetFileName(FilePath)}", FilePath);
            }
        }
